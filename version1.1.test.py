...
def load_csv_data():
    ...
    return taxonomy, dict(persona_map), plr_lookup


TAXONOMY, PERSONA_MAP, PLR_LOOKUP = load_csv_data()
...
if not personas_for_selection:
    ...
    for row_idx in range(rows_needed):
        ...
        for col_idx, col in enumerate(cols):
            ...
            selected_badge = ('<span style="background:#F5A623;color:#000;font-family:\'IBM Plex Mono\','
                              'monospace;font-size:0.48rem;padding:0.1rem 0.35rem;border-radius:2px;'
                              'font-weight:700;letter-spacing:0.08em;">ACTIVE</span>') if is_selected else ""
    ...
    if selected_persona and active_persona_data:
        ...
        <div style="background:#111;border:1px solid #2A2A2A;border-left:3px solid #F5A623;
          ...
          {'<div><div style="font-family:\'IBM Plex Mono\',monospace;font-size:0.52rem;color:#555;text-transform:uppercase;letter-spacing:0.1em;">Signal Window</div><div style="font-size:0.72rem;color:#888;">' + (time_window if time_window else 'Persona default') + '</div></div>' if True else ''}
        </div>""", unsafe_allow_html=True)
